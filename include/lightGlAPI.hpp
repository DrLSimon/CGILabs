    ...
    namespace gl
    {
        ...
        public:
            ...
            void setUniform(const std::string& name, const boost::any& val) const/*{{{*/
            {
                ...
                try
                {
                    setUniform(name, boost::any_cast<glm::vec2>(val));
                    return;
                }
                catch(const boost::bad_any_cast &)
                {
        ...
        public:
            ...
            void resize(int width, int height, float near, float far)/*{{{*/
            {        
                // set the projection matrix
                projectionMatrix = glm::perspective(45.0f, width/(float)height, near, far);
        ...
        {
        public:
            CameraHandler ():rotation(), position(0,0,0), rotationPerTick(0.2), near(1), far(500.)
            {}
            ...
            {
                glm::mat3 rot(rotation);
                return glm::lookAt(position+rot*glm::vec3(0,-10,0), position, rot*glm::vec3(0,0,1));
                
            }/*}}}*/

            void updateCamera()/*{{{*/
            {
                glm::mat4& wv=camera.worldViewMatrix;
                wv=getViewMatrix();
            }/*}}}*/
        ...
        public:
            ...
            std::string performPicking(const Scene& scene, int x, int y) const;

            std::set<std::string> pickableNames;
        private:
            bool intersectInstance(const Scene& scene, const std::string& instancename, const Ray& ray, const glm::mat4& wv, float& distHit) const;
            
        };/*}}}*/

        /** @class IControls {{{
         *  @brief A abstract class for controlling views and other settings
         *  
         *
         */
        class IControls
        ...
        public:
            ...
            virtual void updateProgramUniforms(const Program& prog, const glm::mat4& instanceMW) const=0;
        ...
        {
        public://Methods/*{{{*/
            Controls(unsigned int nbEx):IControls(), wireframe(false), controlCam(true), nbExercises(nbEx), exercise(0), rotateLY(0), rotateLZ(0), lightdirn(0., 0.8, 0.5), lightcolor(glm::vec3(0.6, 0.4, 0.4)), ambient(glm::vec3(0.4, 0.4, 0.4)), oldtime(0), time(0){/*{{{*/
                modelWorldMatrix.push(glm::mat4());
                worldViewMatrix.push(glm::mat4());
            }/*}}}*/
            ...
            {
                cameraHandler.camera.window=window;
            }

            GLFWwindow* getWindow() const
            {
                return cameraHandler.camera.window;
            }

            const Camera& getCamera() const
            {
                return cameraHandler.camera;
            }

            glm::mat4 getWorldMatrix() const
            {
                return modelWorldMatrix.top();
            }

            glm::mat4 getViewMatrix() const
            {
                return worldViewMatrix.top();
            }
                                                                                                  
            const glm::mat4& getProjectionMatrix() const
            {
                return getCamera().projectionMatrix;
            }
            ...
            void updateView()/*{{{*/
            {
                ...
                wv*=cameraHandler.camera.worldViewMatrix;
            ...
            {
                glm::mat4 mw=getWorldMatrix();
                mw=mw*instanceMW;
                glm::mat4 wv=getViewMatrix();
                ...
                prog.setUniform("projectionMatrix", getProjectionMatrix());
                prog.setUniform("modelViewMatrix", wv*mw);
                prog.setUniform("modelWorldMatrix", mw);
                prog.setUniform("worldViewMatrix", wv);
                glm::mat3 normalMatrix= glm::inverse(glm::transpose(glm::mat3(wv*mw)));
            ...
            virtual void keyCallback(int key, int action, int modifiers=0)/*{{{*/
            {
                if(action==GLFW_PRESS)
                {
                    ...
                    case 'R':
                        if(controlCam)
                        {
                            cameraHandler.rotation=glm::mat4();
                            cameraHandler.position=glm::vec3(0,0,0);
                        }
                        else
                            rotateLY=rotateLZ=0;
                        break;
            ...
            {
                float delta_rotate = (time - oldtime) * cameraHandler.rotationPerTick * 2*glm::pi<float>();
                GLFWwindow* window=getCamera().window;
            ...
            {
                glm::vec3 lightdirn=glm::normalize(glm::mat3(getViewMatrix())*getLightDir());
                return lightdirn;
            }/*}}}*/
        ...
        public:
            ...
            //!note: the distHit is not modified since it would automatically be
            //!      smaller than the distance to the real object
        ...
        public:
            ...
            void makeMePlane(const glm::vec4& color=glm::vec4(1,1,1,1))/*{{{*/
            {
                m_verts.clear();
                m_verts.push_back(Vertex({-1,0,-1}, {0,1,0}, {0,0}, color, {1,0,0}));
                m_verts.push_back(Vertex({1, 0,-1}, {0,1,0}, {1,0}, color, {1,0,0}));
                m_verts.push_back(Vertex({1, 0, 1}, {0,1,0}, {1,1}, color, {1,0,0}));
                m_verts.push_back(Vertex({-1,0, 1}, {0,1,0}, {0,1}, color, {1,0,0}));
                
                m_verts.push_back(Vertex({-1,0,-1}, {0,-1,0}, {0,0}, color, {1,0,0}));
                m_verts.push_back(Vertex({1, 0,-1}, {0,-1,0}, {1,0}, color, {1,0,0}));
                m_verts.push_back(Vertex({1, 0, 1}, {0,-1,0}, {1,1}, color, {1,0,0}));
                m_verts.push_back(Vertex({-1,0, 1}, {0,-1,0}, {0,1}, color, {1,0,0}));
                m_tris={0,1,2, 0,2,3, 4,6,5, 4,7,6};
                computeBSphere();
            }/*}}}*/
